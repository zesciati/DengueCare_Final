...
//            val signupConfirmpassword = binding.ConfirmPassword.text.toString()
            ...
            if (signupFullname.isNotEmpty()  && signupNoHp.isNotEmpty() && signupUsername.isNotEmpty() && signupPassword.isNotEmpty() ){
                signupUser(signupFullname,signupNoHp,signupUsername,signupPassword,dataImage)
            } else{
    ...
    private fun signupUser(fullname: String,username: String,password: String,email: String,noHp: String){
        databaseReference.orderByChild("username").equalTo(username).addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                if(!dataSnapshot.exists()){
                    val id = databaseReference.push().key
                    val userData = UserData(id,fullname,email,noHp,username,password )
